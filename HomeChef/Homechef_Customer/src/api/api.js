/* ──────────────────────────────────────────────────────────
   ...
   const api = axios.create({
     baseURL: import.meta.env.VITE_API ?? "https://localhost:7019/api",
     headers: {
   ...
   export const addFavorite              = (recipeId)    => api.post  (`/Favorites/${recipeId}/favorite`, {});
   export const removeFavorite           = (recipeId)    => api.delete(`/Favorites/${recipeId}/favorite`);
   
   /* =========================================================
      4.  REVIEWS
      ========================================================= */
   export const fetchReviews  = (recipeId)           => api.get (`/reviews/${recipeId}`);
   export const addReview     = (recipeId, text)     => api.post(`/reviews/${recipeId}`, text);
   export const updateReview  = (reviewId, text)     => api.put (`/reviews/${reviewId}`, text);
   export const deleteReview  = (reviewId)           => api.delete(`/reviews/${reviewId}`);
   
   /* =========================================================
      5.  RATINGS
      ========================================================= */
   export const postRating       = (recipeId, rating) => api.post ("/ratings", { recipeId, rating });
   export const updateRating     = (recipeId, rating) => api.put  ("/ratings", { recipeId, rating });
   export const deleteRating     = (recipeId)         => api.delete("/ratings", { data: { recipeId } });
   ...
   export const updateMyRecipe = (recipeId, updatedData)       => api.put (`/myrecipes/${recipeId}`,   updatedData);
   ...
   export const updateUserProfilePicture = (data)         => api.put ("/userprofile/update-picture",  data);
   export const updatePassword           = (data)         => api.put ("/userprofile/change-password", data);
   ...
   export const fetchPexelsImage = async (query) => {
     try {
       const res = await api.get(`/gemini/search?query=${encodeURIComponent(query)}`);
